...
class _CustomTabButtonState extends State<CustomTabButton> with SingleTickerProviderStateMixin {
  ...
  void initState() {
    ...
    _textScaleAnimation = Tween<double>(begin: 1.0, end: 0.95).animate(CurvedAnimation(parent: _controller, curve: Curves.easeInOut));
  }
  ...
  @override
  void didUpdateWidget(CustomTabButton oldWidget) {
            ...
            child: Container(
              ...
              decoration: BoxDecoration(
                gradient: LinearGradient(
                  colors: widget.disableGradient ? [AppColors.textWhite, AppColors.textWhite] : [widget.isOutline != null ? widget.enabledColor : AppColors.textBlack, widget.isOutline != null ? widget.enabledColor : AppColors.textBlack],
                  begin: Alignment.topCenter,
                  end: Alignment.bottomCenter,
                ),
                // color: buttonColor,
                borderRadius: BorderRadius.circular(widget.borderRadius ?? 27.5),
                border:
                    widget.isEnabled
                        ? widget.isOutline
                            ? Border.all(color: widget.outlineColor ?? Colors.black, width: widget.outlineWidth)
                            : null
                        : null,
              ),
              ...
              child: Row(
                ...
                children: [
                  ...
                  if (widget.isLoading) FadeTransition(opacity: _opacityAnimation, child: SizedBox(height: 24, width: 24, child: CircularProgressIndicator(color: widget.loadingColor, strokeWidth: 1))),
                  if (widget.isDoctorView) Image.asset(ImagePath.subqdocs_icon, height: 24, width: 24),
                  if (widget.isLoading) const SizedBox(width: 8),
                  // Space between spinner and text
                  // Button Text with ScaleTransition
                  Transform.scale(scale: _textScaleAnimation.value, child: Text(widget.isLoading ? "Please wait" : widget.text, style: AppFonts.medium(widget.fontSize ?? 17, widget.isEnabled ? widget.enabledTextColor : AppColors.textPurple.withValues(alpha: 0.5)))),
                ],
              ),
            ),
          );
