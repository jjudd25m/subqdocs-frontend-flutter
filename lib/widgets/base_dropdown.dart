  ...
  Widget build(BuildContext context) {
    return DropdownButtonHideUnderline(
      child: DropdownButton2<T>(
        ...
        items: items.map((item) => DropdownMenuItem(value: item, child: Padding(padding: const EdgeInsets.symmetric(vertical: 2), child: Text(valueAsString(item), maxLines: 2, style: AppFonts.regular(16, AppColors.textBlack))))).toList(),
        value: selectedValue,
        style: AppFonts.medium(14, AppColors.textBlack),
        onChanged: onChanged,
        buttonStyleData: ButtonStyleData(padding: const EdgeInsets.only(right: 5), height: 48.5, decoration: decoration ?? BoxDecoration(color: AppColors.white, borderRadius: BorderRadius.circular(5), border: Border.all(color: AppColors.textDarkGrey, width: 0.5))),
