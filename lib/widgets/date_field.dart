...
class DateFormatField extends StatefulWidget {
  const DateFormatField({super.key, this.checkValidation, required this.onComplete, required this.type, this.addCalendar = true, this.decoration, this.controller, this.initialDate, this.firstDate, this.lastDate, this.focusNode});
...
class _DateFormatFieldState extends State<DateFormatField> {
  ...
  InputDecoration? decoration() {
    ...
    if (widget.decoration == null) {
      return InputDecoration(
        // border: OutlineInputBorder(borderRadius: BorderRadius.circular(6), borderSide: BorderSide(color: AppColors.textDarkGrey.withAlpha(127), width: 0.5)),
        // enabledBorder: OutlineInputBorder(borderRadius: BorderRadius.circular(6), borderSide: BorderSide(color: AppColors.textDarkGrey.withAlpha(127), width: 0.5)),
        // focusedBorder: OutlineInputBorder(borderRadius: BorderRadius.circular(6), borderSide: BorderSide(color: AppColors.textDarkGrey.withAlpha(127), width: 0.5)),
        ...
        suffixIcon: IconButton(onPressed: pickDate, icon: const Icon(Icons.calendar_month)),
      );
    }

    return widget.decoration!.copyWith(suffixIcon: IconButton(onPressed: pickDate, icon: const Icon(Icons.calendar_month)));
  }
  ...
  Future<void> pickDate() async {
    ...
    final picked = await showDatePicker(context: context, initialDate: widget.initialDate, firstDate: widget.firstDate ?? DateTime(1000), lastDate: widget.lastDate ?? DateTime(3000));
    if (picked != null) {
      String inputText;
      switch (widget.type) {
        case DateFormatType.type2:
  ...
  Widget build(BuildContext context) {
    return TextFormField(
      ...
      onTap: () {
        _dobFormater.selection = TextSelection.fromPosition(TextPosition(offset: _dobFormater.text.length));
      },
