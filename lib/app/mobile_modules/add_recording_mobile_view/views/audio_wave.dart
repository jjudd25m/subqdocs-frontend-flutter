...
class AudioWave extends StatefulWidget {
  const AudioWave({required this.bars, this.height = 100, this.width = 200, this.spacing = 5, this.alignment = 'center', this.animation = true, this.animationLoop = 0, this.beatRate = const Duration(milliseconds: 200), super.key});
      ...
      child: Row(
        children: [
          Wrap(
            ...
            children: [
              if (bars != null)
                for (final bar in bars!) Container(height: bar.heightFactor * widget.height, width: width, decoration: BoxDecoration(gradient: bar.gradient, color: bar.color, borderRadius: BorderRadius.circular(bar.radius))),
            ],
          ),
        ],
      ),
