...
class PatientProfileController extends GetxController {
  ...
  void onInit() {
    ...
    customPrint("PatientProfileController init called");
  ...
  Future<void> getPatient(String id, String visitId) async {
    ...
    try {
      ...
      customPrint("PatientDetailModel :- ${localPatientDetailModel.toJson()}");

      patientDetailModel.value = localPatientDetailModel;

      customPrint(
        "patientDetailModel-----:- ${patientDetailModel.value?.toJson()}",
      );

      // Parse the date string to a DateTime object
      DateTime dateTime =
          DateTime.parse(
            patientDetailModel.value?.responseData?.dateOfBirth ?? "",
          ).toLocal();
  ...
  Future<void> patientScheduleCreate({
    required Map<String, dynamic> param,
  }) async {
    PatientScheduleModel response = await _homeRepository.patientVisitCreate(
      param: param,
    );
