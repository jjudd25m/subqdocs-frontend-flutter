...
class NestedDraggableTableState extends State<NestedDraggableTable> {
        ...
        children: [
          ...
          Expanded(
            child: SingleChildScrollView(
              ...
              child: Column(children: [...List.generate(widget.tableModel.rows.length, _buildRow), _buildNewProcedureAlternative(), _buildFooterRow()]),
            ),
          ),
        ],
      ),
                        ...
                        LongPressDraggable<int>(
                          ...
                          child: Container(
                            ...
                            decoration: BoxDecoration(
                              ...
                              boxShadow: [
                                BoxShadow(color: Colors.grey.withOpacity(0.5), spreadRadius: 2, blurRadius: 5, offset: const Offset(0, 3)), // changes position of shadow
                              ],
                            ),
                ...
                if (colIndex == 0) {
                  if (widget.tableModel.rows[rowIndex].popoverController.opened) {
                    ...
                    Future.delayed(const Duration(milliseconds: 250), () {
                      widget.tableModel.rows[rowIndex].popoverController.open();
                    });
                  }
                }
              },
              child: _buildCell(rowIndex, colIndex, isDiagnosis: colIndex == 1),
            );
          }),
        ),
      ],
                    ...
                    (col == 2)
                        ? Container(
                          ...
                          child: TextFormField(
                            ...
                            onChanged: (value) {
                              print("onchanged called for col == 2");
                                                                      ...
                                                                      for (int rows = 0; rows < widget.tableModel.rows.length; rows++) {
                                                                        for (int cols = 0; cols < widget.tableModel.rows[rows].cells.length; cols++) {
                                                                          for (int newItems = 0; newItems < widget.tableModel.rows[rows].cells[cols].items.length; newItems++) {
                                                                            for (
                                                                              int diag = 0;
                                                                              diag < (widget.tableModel.rows[rows].cells[cols].items[newItems].diagnosisModelList?.length ?? 0);
                                                                              diag++
                                                                            ) {
          ...
          return Padding(
            ...
            child:
                (col == 2)
                    ...
                    : (col == 3)
                    ? IgnorePointer(
                      child: InlineEditableText(onChanged: (p0) {}, initialText: "${items[i].unitPrice}", textStyle: AppFonts.regular(14, AppColors.textGreyTable), onSubmitted: (newText) {}),
                    )
                    : isDiagnosis
                    ? Container(
                      child: Column(
  ...
  Future<void> calculateTotal() async {
    ...
    for (var row in widget.possibleDignosisProcedureTableModel.rows) {
      ...
      final procedure1 = createProcedure(code: row.cells.first.items[0].code ?? "", description: row.cells.first.items[0].description ?? "", modifier: "");
...
class CurrencyInputFormatter extends TextInputFormatter {
  ...
  TextEditingValue formatEditUpdate(TextEditingValue oldValue, TextEditingValue newValue) {
    ...
    // If the new text is empty after removing non-digits, return just the dollar sign
    if (newText.isEmpty) {
      return newValue;
      // return const TextEditingValue(text: '\$', selection: TextSelection.collapsed(offset: 1));
    }

    // Add the dollar sign prefix
    ...
    // Ensure cursor doesn't go before the dollar sign
    if (selectionIndex < 1) {
      selectionIndex = 1;
    }
