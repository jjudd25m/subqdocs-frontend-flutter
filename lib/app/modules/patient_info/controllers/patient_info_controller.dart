...
class PatientInfoController extends GetxController with WidgetsBindingObserver {
  ...
  Future<void> getPatientDetails() async {
    patientData.value = await _visitMainRepository.getPatientDetails(
      id: visitId,
    );

    print("patient data is :- ${patientData.toString()}");
  ...
  void onInit() async {
    ...
    if (Get.arguments["trascriptUploadData"] != null) {
      ...
      getPatientDetails();
      customPrint(
        "patientTranscriptUploadModel data is:- \n \n ${patientTranscriptUploadModel.toJson()}",
      );
    }
  }
    ...
    if (patientTranscriptUploadModel.responseData != null) {
      ...
      if (socketService.socket.connected) {
        ...
        socketService.socket.emit("joinRoom", [
          loginData.responseData?.user?.id,
          patientTranscriptUploadModel.responseData?.visitId,
        ]);
  ...
  Future<void> updateDoctorViewAPI(
    String id,
    Map<String, dynamic> param,
  ) async {
    dynamic response = await _patientInfoRepository.updateDoctorView(
      id: id,
      params: param,
    );
  ...
  Future<void> changeStatus() async {
    ...
    ChangeStatusModel changeStatusModel = await _patientInfoRepository
        .changeStatus(id: visitId, params: param);
  ...
  Future<void> getTranscript() async {
    transcriptListModel.value = await _patientInfoRepository.getTranscript(
      id: visitId,
    );
    getPatientDetails();
    customPrint(
      "transcriptListModel is :- ${transcriptListModel.value?.toJson()}",
    );
  }

  Future<void> getPatientView() async {
    patientViewListModel.value = await _patientInfoRepository.getPatientView(
      id: visitId,
    );
  ...
  Future<Icd10CodeListModel> getIcd10CodeList(
    Map<String, dynamic> param,
  ) async {
  ...
  Future<void> getPatientDoctorVisitData() async {
    patientDoctorVisitDataModel.value = await _patientInfoRepository
        .getDoctorVisitData(id: visitId);
    getPatientDetails();
    customPrint(
      "getPatientDoctorVisitData is :- ${patientDoctorVisitDataModel.value?.toJson()}",
    );
  }

  Future<void> getFullNote() async {
    patientFullNoteModel.value = await _patientInfoRepository.getFullNote(
      id: visitId,
    );
  ...
  Future<void> updateImpressionAndPlan() async {
    ...
    print(params);
    try {
      var response = await _patientInfoRepository.updateImpressionAndPlan(
        id: doctorViewList.value?.responseData?.id ?? 0,
        params: params,
      );
    } catch (e) {
      print(e);
  ...
  Future<void> updateImpressionAndPlanFullNote() async {
    ...
    try {
      var response = await _patientInfoRepository
          .updateImpressionAndPlanFullNote(
            id: patientFullNoteModel.value?.responseData?.id ?? 0,
            params: params,
          );
    } catch (e) {
      print(e);
  ...
  Future<void> updateFullNote(
    ...
    var response = await _patientInfoRepository.updateFullNote(
      id: patientFullNoteModel.value?.responseData?.id ?? 0,
      params: buildParams(keyName, list),
    );
  }

  Future<void> updatePatientView(
    String keyName,
    List<ImpresionAndPlanViewModel> list,
  ) async {
    var response = await _patientInfoRepository.updatePatientView(
      id: patientViewListModel.value?.responseData?.id ?? 0,
      params: buildParams(keyName, list),
    );
  }
  ...
  Future<void> getDoctorNote() async {
    ...
    doctorViewList.value = await _patientInfoRepository.getDoctorNote(
      id: visitId,
    );
    setImpressionAndPlanList();
    setDoctorModel();
  ...
  void setDoctorModel() {
    ...
    for (MainDiagnosisCodesProceduresDiagnosisCodesProcedures diagnosis
        in (doctorViewList
                .value
                ?.responseData
                ?.mainDiagnosisCodesProcedures
      ...
      for (Diagnosis diagnosisModel in diagnosis.diagnosis ?? []) {
        print(
          "diagnosis aleternative:- ${diagnosisModel.diagnosisPossibleAlternatives?.length ?? 0}",
        );
        ...
        diagnosisModelList.add(
          DiagnosisModel(
            ...
            diagnosisPossibleAlternatives:
                diagnosisModel.diagnosisPossibleAlternatives,
          ),
        );
      }
            ...
            TableCellModel(
              items: [
                SingleCellModel(
                  ...
                  unitPrice: "0",
                  procedurePossibleAlternatives:
                      diagnosis.procedure?.procedurePossibleAlternatives,
                ),
              ],
            ),

            TableCellModel(
              items: [SingleCellModel(diagnosisModelList: diagnosisModelList)],
            ),

            TableCellModel(items: [SingleCellModel(unit: diagnosis.units)]),
            TableCellModel(
              items: [SingleCellModel(unitPrice: diagnosis.unitCharge)],
            ),
          ],
        ),
      );
    ...
    for (PossibleDiagnosisCodesProcedures possibleDiagnosisCodesProcedures
        in (doctorViewList
                .value
                ?.responseData
                ?.mainDiagnosisCodesProcedures
                ?.possibleDiagnosisCodesProcedures ??
            [])) {
      List<DiagnosisModel>? diagnosisModelList = [];

      for (Diagnosis diagnosisModel
          in possibleDiagnosisCodesProcedures.diagnosis ?? []) {
        print(
          "diagnosis aleternative:- ${diagnosisModel.diagnosisPossibleAlternatives?.length ?? 0}",
        );
      ...
      possibleDignosisProcedureTableModel.value?.rows.add(
        TableRowModel(
          cells: [
            TableCellModel(
              items: [
                SingleCellModel(
                  ...
                  unit: "0",
                  modifiers:
                      possibleDiagnosisCodesProcedures.procedure?.modifier,
                  description:
                      possibleDiagnosisCodesProcedures.procedure?.description ??
                      "",
                  unitPrice: "0",
                  procedurePossibleAlternatives:
                      possibleDiagnosisCodesProcedures
            ...
            TableCellModel(
              items: [
                SingleCellModel(unit: possibleDiagnosisCodesProcedures.units),
              ],
            ),
            TableCellModel(
              items: [
                SingleCellModel(
                  unitPrice: possibleDiagnosisCodesProcedures.unitCharge,
                ),
              ],
            ),
          ],
  ...
  void offLine() {
    ...
    patientFullNoteModel.value = PatientFullNoteModel.fromJson(
      fullNoteResponse,
    );
    patientDoctorVisitDataModel.value = PatientDoctorVisitDataModel.fromJson(
      visitDataResponse,
    );
    transcriptListModel.value = TranscriptListModel.fromJson(
      fullTransScriptResponse,
    );
    patientViewListModel.value = PatientViewListModel.fromJson(
      patientViewResponse,
    );
  ...
  void setImpressionAndPlanListPatientView() {
    ...
    if (patientFullNoteModel
      ...
      editableDataForSkinHistory.add(
        ImpresionAndPlanViewModel(
          htmlContent:
    ...
    if (patientFullNoteModel
      ...
      editableDataForCancerHistory.add(
        ImpresionAndPlanViewModel(
          htmlContent:
              patientFullNoteModel
                  .value
    ...
    if (patientFullNoteModel
      ...
      editableDataForSocialHistory.add(
        ImpresionAndPlanViewModel(
          htmlContent:
              patientFullNoteModel
                  .value
    ...
    if (patientFullNoteModel.value?.responseData?.fullNoteDetails?.allergies !=
        null) {
    ...
    if (patientFullNoteModel
      ...
      editableDataForReviewOfSystems.add(
        ImpresionAndPlanViewModel(
          htmlContent:
              patientFullNoteModel
                  .value
    ...
    if (patientFullNoteModel.value?.responseData?.fullNoteDetails?.exam !=
        null) {
      editableDataForExam.clear();

      HtmlEditorController htmlEditorController = HtmlEditorController();
      // htmlEditorController.setText(patientFullNoteModel.value?.responseData?.fullNoteDetails?.exam ?? "");
      editableDataForExam.add(
        ImpresionAndPlanViewModel(
          htmlContent:
              patientFullNoteModel.value?.responseData?.fullNoteDetails?.exam ??
              "",
          htmlEditorController: htmlEditorController,
          title: "",
        ),
    ...
    if (patientFullNoteModel.value?.responseData?.fullNoteDetails?.hpi !=
        null) {
      editableDataHpiView.clear();

      HtmlEditorController htmlEditorController = HtmlEditorController();
      // htmlEditorController.setText(patientFullNoteModel.value?.responseData?.fullNoteDetails?.hpi ?? "");
      editableDataHpiView.add(
        ImpresionAndPlanViewModel(
          htmlContent:
              patientFullNoteModel.value?.responseData?.fullNoteDetails?.hpi ??
              "",
          htmlEditorController: htmlEditorController,
          title: "",
        ),
  ...
  void setPatientViewEditableData() {
    if (patientViewListModel
      ...
      editableDataForPatientView.add(
        ImpresionAndPlanViewModel(
          htmlContent:
              """ <section style='font-size:16px; font-family: Poppins, sans-serif; color:#757575; font-weight: normal;'><p><strong>General Appearance:</strong> Well-appearing in no acute distress </p><p><strong>Skin:</strong></p><ul><li><strong>Forehead:</strong> Inflamed, purple, squishy acne cyst approximately 0.9 centimeters in size </li></ul></section>  """,
          htmlEditorController: htmlEditorController,
          title: "",
          initialHtmlContent:
...
//     isKeyboardOpen.value = viewInsets.bottom > 0;
//   }
// }
...
class KeyboardController extends GetxController {
  ...
  void checkKeyboard() {
    final currentHeight = Get.mediaQuery.size.height;
    final keyboardHeight = screenHeight - currentHeight;
