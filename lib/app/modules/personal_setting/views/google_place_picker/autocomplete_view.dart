...
class PlacesAutocomplete extends StatefulWidget {
  ...
  const PlacesAutocomplete({
    ...
    this.language,
    this.topCardMargin = const EdgeInsets.all(0),
    this.topCardColor,
    this.topCardShape = const RoundedRectangleBorder(borderRadius: BorderRadius.all(Radius.circular(8))),
...
class _PlacesAutocompleteState extends State<PlacesAutocomplete> {
  ...
  AutoCompleteState autoCompleteState() {
    return AutoCompleteState(apiHeaders: widget.placesApiHeaders, baseUrl: widget.placesBaseUrl, httpClient: widget.placesHttpClient);
  }
  ...
  void _getDetailsByPlaceId(String placeId, BuildContext context) async {
    try {
      final GoogleMapsPlaces places = GoogleMapsPlaces(apiKey: widget.apiKey, httpClient: widget.placesHttpClient, apiHeaders: widget.placesApiHeaders, baseUrl: widget.placesBaseUrl);
      final PlacesDetailsResponse response = await places.getDetailsByPlaceId(placeId, region: widget.region, sessionToken: widget.sessionToken, language: widget.language, fields: widget.fields);
