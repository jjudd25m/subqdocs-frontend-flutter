...
class ForgotPasswordController extends GetxController {
  ...
  Map<String, Widget> screenMap = {"otp_Screen": const EnterOtpView(), "enter_password": const EnterPasswordView(), "password_changed_screen": const PasswordChangedScreen()};
  ...
  Widget getCurrentScreen() {
    return screenMap[currentScreen.value] ?? const EnterMailView(); // Default screen if no match
  }
  ...
  Future<void> sendOtp() async {
    ...
    try {
      ...
      if (sendOtpModel.responseData != false) {
        ...
        customPrint("response is ${sendOtpModel.toJson()} ");
  ...
  Future<void> verifyOtp() async {
    ...
    try {
      ...
      if (verifyOtpModel.responseData != false) {
        isLoading.value = false;
        customPrint(verifyOtpModel.toJson());
