...
class RecorderService {
  ...
  Future<bool> startRecording(BuildContext context) async {
    ...
    if (btMic && btGranted) {
      ...
      _pcmSubscription = _pcmStreamController.stream.listen((buffer) {
        ...
        globalController.samples.add(AudioWaveBar(heightFactor: (amplitude * 2).clamp(0.0, 1.0), color: Colors.purple));
  ...
  Future<String> stopRecording() async {
    ...
    Loader().stopLoader();
    return globalController.visitId.value;
  }
    ...
    try {
      ...
      if (connectivityResult.contains(ConnectivityResult.none)) {
        ...
        if (response.isNotEmpty) {
          ...
          Map<String, dynamic> params = {};
          params["session_id"] = sessionId;
          Map<String, dynamic> responses = await visitMainRepository.uploadLastRecord(visitId: visitId, params: params, chunkFile: chunkFile, mimeType: mimeType ?? "", token: loginData.responseData?.token ?? "");
