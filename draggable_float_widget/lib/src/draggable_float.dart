...
class DraggableFloatWidget extends StatefulWidget {
  ...
  final void Function(double, double) receiveParam;
...
class _CustomDraggableFloatState extends State<DraggableFloatWidget> with TickerProviderStateMixin<DraggableFloatWidget> {
  ...
  _initBorderSize() {
    ...
    _bottom = _screenHeight - widget.height - widget.config.borderBottom - _bottomBarHeight - (widget.config.isFullScreen ? 0 : _variableTop); // The bottom bar height of the full screen needs to be subtracted.
    ...
    positionX = widget.config.valueOfTheX != 0
        ? widget.config.valueOfTheX
        : widget.config.initPositionXInLeft
            ? _left
            : _right;
    positionY = widget.config.valueOfThey != 0
        ? widget.config.valueOfThey
        : widget.config.initPositionYInTop
  ...
  _handleScrollEvent(OperateEvent event) {
    ...
    if ((currentState == DraggableFloatWidgetState.SHOW && event == OperateEvent.OPERATE_SHOW) || (currentState == DraggableFloatWidgetState.HIDE && event == OperateEvent.OPERATE_HIDE)) return;
